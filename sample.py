...
tf.app.flags.DEFINE_string('data_path','access.log',"path of log")
            ...
            if (ips.has_key(ip)):
                ...
                if (dt<1800):
                    if (pid.indexj<28):
                        ...
                        attackerIP.append(ip)
                        attackerTime.append(t)
                else:
                    person = [0 for i in range(784)]
                    trun_person(person, 0, 0, findurls(url), hash(browser) % 1007)
                    pid = ipaddr()
...
result = sess.run(y, feed_dict={x: data,y: label})
for i in range(len(result)):
    if (result[i][0]!=1):
        print "IP为",attackerIP[i],"用户在",attackerTime[i],"左右发动了",
        if result[i][1]==1: print"DDos"
        if result[i][2]==1: print"爆破"
        if result[i][3]==1: print"撞库"